...
export function ThemeToggle({ className }: { className?: string }) {
    ...
    if (!mounted) {
        return <div className={cn("w-9 h-9 rounded-xl bg-muted animate-pulse", className)} />;
    }
    ...
    return (
        <button
            ...
            className={cn(
                "flex items-center justify-center w-9 h-9 rounded-xl bg-muted hover:bg-accent border border-border transition-all group",
                className
            )}
